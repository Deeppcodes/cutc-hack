...
import { ForYouNote } from "@/components/ForYouNote";
...
export function ForecastDetail({ market }: { market: Market }) {
  ...
  const [forYou, setForYou] = React.useState<string | undefined>();
  ...
  const runLive = React.useCallback(async () => {
    ...
    try {
      const result = await fetchForecast(market.id, lens);
      setForecast(result.forecast);
      setForYou(result.forYou);
    } finally {
      clearInterval(ticker);
      setRunning(false);
    }
    ...
    <div className="mx-auto max-w-[1240px] px-5 pb-24 pt-8 lg:px-8">
      ...
      <div className="mt-7 grid gap-px overflow-hidden rounded-[14px] border border-[#1e232c] bg-[#1e232c] sm:grid-cols-3">
        <div className="bg-[#101318] px-6 py-6">
          <ProbabilityStat
            ...
            footer={
              <div
                className={cn(
                  "flex items-center gap-1.5 text-[13px] font-medium tnum",
        ...
        <div className="bg-[#101318] px-6 py-6">
          <ProbabilityStat
            tone="contrary"
            label="Contrary forecast"
            value={forecast.probability}
            size="lg"
            animate
            footer={
              <div className="flex items-center gap-2 text-[13px] text-[#646c7a]">
        ...
        >
          ...
          <div
            className={cn(
              "mt-1.5 text-[52px] font-semibold leading-none tracking-[-0.03em] tnum",
